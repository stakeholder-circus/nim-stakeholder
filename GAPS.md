...
- `nim-stakeholder.experimental-provider-pending`: live-provider adapters are intentionally absent; `--experimental-provider` fails fast explicitly.
- `nim-stakeholder.browser-session-pending`: browser/session capture and credentialed integration harnesses are not implemented in Nim.
